    ...
    var body: some View {
        VStack {
            ScrollView {
                ...
                let webviewCommand = WebviewCommand(config: "webview open config")
                let safariCommand = SafariCommand(config: "safari open config")
                
                let btn1 = RealCommandButton(command: webviewCommand).then {
                    $0.backgroundColor = .red
                }
                let btn2 = RealCommandButton(command: safariCommand).then {
...
final class CommandButtonClass {
